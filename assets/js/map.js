  ...
  try {
    ...
    if (national) {
      ...
      response = await fetch(
        `${BASE}/${fRegion}/${fProvince.toLowerCase()}/${fCity.toLowerCase()}.json`
      );
      response = await response.json();
      ...
      if (fRegion === "national_capital_region") {
        const group = (data) => {
          ...
          for (const { CANDIDATE_NAME, TOTAL_VOTES } of data) {
            if (voteMap.has(CANDIDATE_NAME)) {
              voteMap.set(
                CANDIDATE_NAME,
                voteMap.get(CANDIDATE_NAME) + TOTAL_VOTES
              );
            } else {
          ...
          const summedVotes = Array.from(voteMap.entries()).map(
            ([name, votes]) => ({
              CANDIDATE_NAME: name,
              TOTAL_VOTES: votes,
            })
        ...
        const ncrResults = allLocations.regions
          .find((item) => item.code === fRegion)
        ...
        const obj = {
          ...
          results: {
            "SENATOR OF PHILIPPINES": group(
              data.map((item) => item.results["SENATOR OF PHILIPPINES"]).flat()
            ),
            "PARTY LIST OF PHILIPPINES": group(
              data
                .map((item) => item.results["PARTY LIST OF PHILIPPINES"])
                .flat()
            ),
          },
        };
    ...
    try {
      ...
      let allRequest = allLocations?.regions?.map(async (item) => {
        ...
        const fRes = item?.places?.map(async (item2) => {
          let name = item2?.name?.includes("-")
            ? item2?.name
                ?.toLowerCase()
                ?.replace("-", "")
                ?.split(" ")
                ?.join("_")
            : item2?.name?.toLowerCase()?.split(" ").join("_");
    ...
    addLayer(
      ...
      function (feature, layer) {
        ...
        provincesLayer.eachLayer(async (provLayer) => {
          ...
          if (
            ...
            try {
              ...
              const { results, timestamp } = await queryData({
                region: payload,
                province:
                  cities[provLayer.feature.properties.NAME_1.toUpperCase()],
              });
          ...
          provLayer.setStyle({
            fillColor:
              provLayer.feature.properties.NAME_1 ===
                feature.properties.NAME_1 && topCandidate
...
async function highlightProvinceOnMap(
  provinceName,
  cityName,
  regionName = null
) {
  ...
  provincesLayer.eachLayer((layer) => {
    ...
    if (
      layerProvince &&
      layerProvince.toLowerCase() ===
        provinceName.replace(/[\s-]+/g, "").toLowerCase()
    ) {
  ...
  if (!found) {
    ...
    console.log(
      "Available provinces:",
      allProvincesData.features.map((f) => f.properties.NAME_1)
    );
  }
}
...
function showCitiesForNCR() {
  ...
  const ncrCities = {
    ...
    features: allCitiesData.features.filter((feature) =>
      (feature.properties.NAME_1 || "")
        .toLowerCase()
        .includes("metropolitanmanila")
    ),
  };
  ...
  citiesLayer = L.geoJSON(ncrCities, {
    style: function (feature) {
      ...
      const cityCandidates = citySenatoriablesData.filter(
        (c) => c.city.toLowerCase().replace(/\s+/g, "") === normalizedCity
      );
...
function highlightCityOnMap(cityName, provinceName, region) {
  ...
  if (
    normalizeName(region) === "nationalcapitalregion" ||
    normalizeName(region) === "ncr"
  ) {
    provinceName = "MetropolitanManila"; // Match exact NAME_1 value in GeoJSON
  }

//   console.log("Normalized Province:", provinceName);
  ...
  const tryHighlightCity = () => {
    ...
    let normalizedCity = normalizeName(cityName);
    // console.log(normalizedCity);
    const normalizedProvince = normalizeName(provinceName);

    // Step 1: Try to update normalizedCity based on partial word match with NAME_2
    for (const layer of citiesLayer.getLayers()) {
      const layerCity = layer.feature?.properties.NAME_2;
      if (!layerCity) continue;

      const pattern = new RegExp(`${normalizedCity}`, "i");
      if (pattern.test(layerCity)) {
        // console.log(
        //   `Updating normalizedCity from "${normalizedCity}" to "${layerCity}"`
        // );
        normalizedCity = layerCity;
        break; // this ensures we stop once we find the match
      }
    }
    // console.log("this final", normalizedCity);
    // Find the exact matching city layer
    const targetLayer = citiesLayer.getLayers().find((layer) => {
      const layerCity = layer.feature?.properties.NAME_2;
      const layerProvince = normalizeName(layer.feature?.properties.NAME_1);
		// console.log("Sample",layerCity, layerProvince);
		// console.log("Sample",normalizedCity, normalizedProvince);
      return (
        layerCity === normalizedCity && layerProvince === normalizedProvince
      );
    });
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (!READY.status) {
      ...
      label.innerHTML =
        "We are still waiting for the data to become available. Kindly check back.";
      label.style =
        "font-size: 16px; position: relative; text-align: center; width: 80%; padding: 200px 0; margin: 0 auto;";

      document.querySelector("#senator-list-btn").style =
        "display: none !important;";
      document.querySelector("#partylist-list-btn").style =
        "display: none !important;";
          ...
          if (nameProperty === "NAME_1") {
            ...
            let HEX =
              staticCandidate?.find((item2) => {
                if (
                  item2?.CANDIDATE_NAME.toLowerCase() ===
                  target?.results[
                    "SENATOR OF PHILIPPINES"
                  ][0]?.CANDIDATE_NAME.toLowerCase()
                ) {
                  return item2;
                }
              })?.HEX_CODE || "#fff";
        ...
        onEachFeature: function (feature, layer) {
          ...
          layer.on({
            mouseover: function (e) {
              ...
              let regionName = `${normalizedName
                ?.charAt(0)
                .toUpperCase()}${normalizedName?.slice(1)}`;
              layer.bindTooltip(
                `${regionName}<br>Top Candidate: ${
                  topCandi?.CANDIDATE_NAME
                }<br>Votes: ${topCandi?.TOTAL_VOTES.toLocaleString()}`,
                {
                  permanent: false,
                  direction: "center",
                  className: "candidate-tooltip",
  ...
  if (provinceName === "metropolitan_manila") {
    ...
    const cities = target?.places?.find((item) => {
      if (
        ...
        item?.code === "maguindanao_del_norte"
      ) {
        return item;
      }
    ...
    var fRes = cities?.map(async (item) => {
      ...
      const payload = `${BASE}/${loweredRegion}/${provinceName.replace(
        " ",
        "_"
      )}/${code}.json`;
  ...
  citiesLayer = L.geoJSON(filteredCities, {
    style: function (feature) {
      ...
      const target = results?.find(
        (item) =>
          normalizeCityName(item?.municipal) === normalizeCityName(cityName)
      );
      const topSenate = target?.results["SENATOR OF PHILIPPINES"][0];

      const HEX =
        staticCandidate?.find(
          (item) => item?.CANDIDATE_NAME === topSenate?.CANDIDATE_NAME
        )?.HEX_CODE || "#eb4034";
    ...
    onEachFeature: function (feature, layer) {
      ...
      layer.bindTooltip(
        `${displayCityName}<br>Top Candidate: ${
          topSenate?.CANDIDATE_NAME
        } <br>Votes: ${topSenate?.TOTAL_VOTES.toLocaleString()}`,
        {
          permanent: false,
          direction: "center",
          className: "candidate-tooltip",
      ...
      layer.on({
        ...
        click: async function (e) {
          ...
          const regionRef = allLocations.regions.find(
            (reg) => reg.code === regionName
          );
          const cityCode = Object.values(regionRef.places)
            ...
            .find((x) => {
              return (
                normalizeCityNameToSnakeCase(x) ===
                normalizeCityNameToSnakeCase(
                  cityName === "KalookanCity" ? "CaloocanCity" : cityName
                )
              );
            });

          const provinceRef = Object.values(regionRef.places).find((dist) =>
            dist.subplaces.find((x) => x.code === cityCode)
          );
...
function updateElectionResults(level, name) {
  ...
  if (level === "national") {
    ...
    filteredData = data.filter(
      (candidate) =>
        candidate[level] &&
        candidate[level].toLowerCase().replace(/\s+/g, "") ===
...
document
  .querySelector(".btn-light[onclick*='setView']")
